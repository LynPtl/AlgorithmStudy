...
def cross(for_seed, size):
    ...
    for i in range(1,size-1):
        for j in range(1,size-1):
            sum = grid[i][j] + grid[i+1][j-1] + grid[i+1][j+1] + grid[i-1][j-1] + grid[i-1][j+1]
            dict[sum] += 1
    max_value = max(dict.keys())
    nb_of_crosses_of_max_value = dict[max_value]
    ...
    for row in range(1,size-1):
        for col in range(1,size-1):
            totol = sum(grid[row + dir_row][col + dir_col] for dir_row, dir_col in directions)
            counter[totol] += 1
    if counter:
        max_value = max(counter)
        nb_of_crosses_of_max_value = counter(max_value)
    """
