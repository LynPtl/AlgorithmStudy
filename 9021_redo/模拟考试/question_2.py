...
def f(for_seed, n, upper_bound):
    ...
    for line in square:
        for col in line:
            dict[col] += 1
            allnumber.append(col)
    allnumber = sorted(allnumber)
    result = set()
    for i in dict.keys():
        if dict[i] > 1:
            result.add(i)
    ...
    if result:
        print(f"It is not a good square because it contains duplicates, namely: {" ".join(result)}")
    else:
        print(f"It is a good square.")
        new_square = []
        cur = 0
        for i in range(n):
            curline = allnumber[cur:cur+n]
            new_square.append(curline)
            cur = cur + n
        ordered_square = list(zip_longest(*new_square,fillvalue=""))
        print(
            "Ordering the elements from left to right column, from top to bottom, yields:"
        )
    ...
    if duplicates:
        print(
            "It is not a good square because it contains duplicates, namely: ", end=""
        )
