...
def largest_smallest_prime_factor_in_rotations(n):
    ...
    for i in range(len(num)):
        new_num = num[i:] + num[:i]
        # 为了去掉1
        if int(new_num) > 1 and int(new_num) not in rotation_numbers:
            rotation_numbers.append(int(new_num))
    primes = sieve_of_primes_up_to(max(rotation_numbers))
    for rotation_number in sorted(rotation_numbers):
        for index in range(2,len(primes)):
            if rotation_number % index == 0:
                result[rotation_number] = index
                break
    smallest_prime_factor = max(result.values())
