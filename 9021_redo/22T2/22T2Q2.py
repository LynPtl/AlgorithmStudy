...
-5, -5, 2, -7, -6, 8], 5)
    ...
    for num in sorted(set(L)):
        progression = [num]
        while progression[-1] + q in L:
            progression.append(progression[-1]+q)
        result[len(progression)].append(progression)
    print(f"The longest {q}-arithmetic progressions of members of L are:")
    if result:
        max_length = max(result.keys())
        for progression in sorted(result[max_length],key = lambda item: item[0]):
