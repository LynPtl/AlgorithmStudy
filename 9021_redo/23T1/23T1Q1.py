...
def transform(number):
    ...
    for i in range(len(strn)):
        if strn[i] != '0':
            strn_withoutzero += strn[i]
    strn_withoutzero = strn_withoutzero + strn_withoutzero[0]
    result = ""
    for i in range(len(strn_withoutzero)-1):
        result += (strn_withoutzero[i] * int(strn_withoutzero[i+1]))
    return int(result)
    #sample answer
    """
